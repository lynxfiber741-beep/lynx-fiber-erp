...
THEMES = {
    "Dark Nebula (Default)": {
        "bg": "#0b0f19", "sidebar_bg": "#111827", "text": "#e5e7eb", "heading": "#10b981", "accent": "#3b82f6",
        "card_bg": "#1f2937", "table_th": "#1f2937", "table_td": "#111827", "border": "#374151",
        "input_bg": "#ffffff", "input_text": "#000000", "login_box_border": "#10b981"
    },
    "Light Corporate": {
        "bg": "#f8fafc", "sidebar_bg": "#ffffff", "text": "#1e293b", "heading": "#059669", "accent": "#2563eb",
        "card_bg": "#ffffff", "table_th": "#e2e8f0", "table_td": "#ffffff", "border": "#cbd5e1",
        "input_bg": "#f1f5f9", "input_text": "#0f172a", "login_box_border": "#2563eb"
    },
    "Midnight Crimson": {
        "bg": "#11090b", "sidebar_bg": "#1a0e11", "text": "#fda4af", "heading": "#e11d48", "accent": "#be123c",
        "card_bg": "#281216", "table_th": "#281216", "table_td": "#1a0e11", "border": "#4c1d28",
        "input_bg": "#fff1f2", "input_text": "#4c0519", "login_box_border": "#e11d48"
    },
    "Ocean Wave": {
        "bg": "#0f172a", "sidebar_bg": "#1e293b", "text": "#e0f2fe", "heading": "#0ea5e9", "accent": "#0284c7",
        "card_bg": "#0f172a", "table_th": "#1e293b", "table_td": "#0f172a", "border": "#38bdf8",
        "input_bg": "#f0f9ff", "input_text": "#0c4a6e", "login_box_border": "#0ea5e9"
    }
}
...
if st.session_state['authenticated'] and not st.session_state['portal_mode']:
    with st.sidebar:
        ...
        selected_theme = st.selectbox(
            "Select UI Theme", list(THEMES.keys()),
...
if routing_node in ["📊 Core Analytics Dashboard", "📊 Lynx Dashboard"]:
    ...
    if not all_system_areas:
        ...
        if not base_df.empty:
            ...
            # ==================================================================================
            # 💳 NEW LOGIC: DIRECT/QUICK BILL PAYMENT DRAWER (INSIDE UNPAID/DASHBOARD GRID)
            # ==================================================================================
            unpaid_interactive_df = base_df[base_df['status'].isin(['UNPAID', 'PARTIAL', 'SUSPENDED'])].copy()
            if not unpaid_interactive_df.empty:
                st.write("")
                with st.expander("💳 ⚡ QUICK BILL PAYMENT DRAWER (Pay Selected Unpaid User Instantly)", expanded=True):
                    st.caption("Aap jis filtered user ko pay karna chahte hain, use niche select karein aur billing process karein:")
                    quick_pay_map = {f"❌ [{row['username']}] {row['customername']} — Arrears: Rs. {row['balanceshift']} ({row['area']})": row['username'] for _, row in unpaid_interactive_df.iterrows()}
                    chosen_quick_label = st.selectbox("🎯 Select Customer to Open Bill Payment Form", ["-- Select a Customer from Filtered View --"] + list(quick_pay_map.keys()), key="db_quick_pay_select")
                    
                    if chosen_quick_label != "-- Select a Customer from Filtered View --":
                        quick_resolved_uid = quick_pay_map[chosen_quick_label]
                        q_node_row = unpaid_interactive_df[unpaid_interactive_df['username'] == quick_resolved_uid].iloc[0].to_dict()
                        
                        try:
                            q_base_bill = int(float(str(q_node_row.get('billamount', 0))))
                            q_base_shift = int(float(str(q_node_row.get('balanceshift', 0))))
                        except Exception:
                            q_base_bill = 0
                            q_base_shift = 0
                            
                        st.info(f"📊 Plan Rate: Rs. {q_base_bill:,} | Outstanding Arrears: Rs. {q_base_shift:,} | Current Expiry: {q_node_row.get('expirydate')}")
                        
                        q_col1, q_col2, q_col3 = st.columns(3)
                        with q_col1:
                            q_billing_months = st.selectbox("📅 Duration (Advance Months)", [1, 3, 6, 12], key="db_quick_months")
                        with q_col2:
                            q_pay_method = st.selectbox("Method Profile", ["CASH", "EASYPAISA", "JAZZCASH", "BANK_TRANSFER"], key="db_quick_method")
                        with q_col3:
                            q_discount = st.number_input("🎁 Discount Approved (Rs.)", min_value=0, value=0, step=50, key="db_quick_discount")
                            
                        q_package_total_cost = q_base_bill * q_billing_months
                        q_net_payable = q_package_total_cost + q_base_shift
                        q_final_due = max(q_net_payable - q_discount, 0)
                        
                        q_cash_in = st.number_input("Capital Received From Customer (Rs.)", min_value=0, value=q_final_due, key="db_quick_cash_in")
                        q_future_shift = int(q_final_due - q_cash_in)
                        
                        if q_future_shift <= 0:
                            q_calculated_status = "PAID"
                            q_status_color = "#10b981"
                        elif q_cash_in > 0:
                            q_calculated_status = "PARTIAL"
                            q_status_color = "#f59e0b"
                        else:
                            q_calculated_status = "UNPAID"
                            q_status_color = "#f43f5e"
                            
                        st.markdown(f"""
                        <div class='live-calc-box'>
                            <p>📦 <b>Package Extension Charges ({q_billing_months} Month(s)):</b> Rs. {q_package_total_cost:,}</p>
                            <p>⏮️ <b>Past Arrears Covered:</b> Rs. {q_base_shift:,}</p>
                            <p>🎁 <b>Discount Subtracted:</b> Rs. {q_discount:,}</p>
                            <h4 style='color:{active_theme['accent']};'><b>Net Outstanding Due:</b> Rs. {q_final_due:,}</h4>
                            <hr style='border:1px solid {active_theme['border']};'>
                            <h4>🔮 <b>Auto Post Action State:</b> <span style='color:{q_status_color}; font-weight:bold;'>{q_calculated_status}</span></h4>
                            <p>💾 <b>New Balanceshift/Arrears Log:</b> Rs. {q_future_shift:,}</p>
                        </div>
                        """, unsafe_allow_html=True)
                        
                        if st.button("💳 POST TRANSACTION & ACTIVATE LINE INSTANTLY", use_container_width=True, key="db_quick_post_btn"):
                            q_today_dt = datetime.now()
                            q_current_expiry_str = str(q_node_row.get('expirydate', '')).strip()
                            try:
                                q_old_expiry_dt = datetime.strptime(q_current_expiry_str, "%Y-%m-%d")
                                q_base_dt = q_today_dt if q_old_expiry_dt < q_today_dt else q_old_expiry_dt
                            except Exception:
                                q_base_dt = q_today_dt
                                
                            q_new_expiry = (q_base_dt + relativedelta(months=q_billing_months)).strftime("%Y-%m-%d")
                            q_invoice_uuid = f"INV-{uuid.uuid4().hex[:10].upper()}"
                            
                            with get_db_connection() as conn:
                                with conn.cursor() as cursor:
                                    cursor.execute("""
                                        UPDATE customers SET balanceshift = %s, status = %s, expirydate = %s WHERE username = %s AND tenant_id = %s
                                    """, (q_future_shift, q_calculated_status, q_new_expiry, quick_resolved_uid, st.session_state['tenant_id']))
                                    cursor.execute("""
                                    ...
                                    """, (q_invoice_uuid, quick_resolved_uid, q_node_row.get('customername'), q_node_row.get('area'), q_node_row.get('phone'), datetime.now().strftime("%Y-%m-%d %H:%M:%S"), q_node_row.get('package'), int(q_cash_in), q_future_shift, q_pay_method, int(q_discount), st.session_state['tenant_id']))
                                    
                            insert_activity_log(st.session_state['tenant_id'], st.session_state['username'], "BILL_PAYMENT", f"Dashboard Quick Pay posted Rs. {q_cash_in} for user {quick_resolved_uid}. Status set to {q_calculated_status}, Arrears to Rs. {q_future_shift}.")
                            st.success(f"🎉 Collection Recorded Cleanly! System Class Status: {q_calculated_status} | Extended To: {q_new_expiry}")
                            
                            q_wa_context = {
                                "name": q_node_row.get('customername', ''),
                                "username": quick_resolved_uid,
                                "package": q_node_row.get('package', ''),
                                "paid": int(q_cash_in),
                                "arrears": q_future_shift,
                                "expiry": q_new_expiry,
                                "method": q_pay_method
                            }
                            send_tenant_whatsapp(tenant_meta, q_node_row.get('phone'), "bill_paid", q_wa_context)
                            
                            st.session_state['recent_pdf_bytes'] = generate_receipt_pdf(TENANT_COMPANY_NAME, TENANT_SUPPORT_PHONE, q_invoice_uuid, quick_resolved_uid, q_node_row.get('customername'), q_node_row.get('area'), q_node_row.get('package'), q_cash_in, q_future_shift, q_pay_method)
                            st.session_state['recent_invoice_uuid'] = q_invoice_uuid
                            st.cache_data.clear()
                            st.rerun()
                            
                if 'recent_pdf_bytes' in st.session_state:
                    st.download_button("📥 Download Generated PDF Receipt", data=st.session_state['recent_pdf_bytes'], file_name=f"Receipt_{st.session_state.get('recent_invoice_uuid', 'INV')}.pdf", mime="application/pdf", use_container_width=True, key="db_quick_pdf_download")
            # ==================================================================================
    ...
    with tab_col:
        if not sub_map:
            ...
            if st.button("💳 POST TRANSACTION & EXTEND LINE", use_container_width=True):
                ...
                wa_context = {
                    "name": node_row_dict.get('customername', ''),
                    "username": resolved_uid,
                    "package": node_row_dict.get('package', ''),
                    "paid": int(cash_in),
                    "arrears": future_shift,
                    "expiry": new_expiry,
                    "method": pay_method
                }
            ...
            if not sub_map:
                ...
                with col_st1:
                    if st.button("🛑 TEMPORARILY DISABLE LINE", use_container_width=True):
                        ...
                        wa_context = {
                            "name": node_sr.get('customername', ''),
                            "username": resolved_uid_sr,
                            "package": node_sr.get('package', ''),
                            "arrears": node_sr.get('balanceshift', 0),
                            "expiry": node_sr.get('expirydate', ''),
                            "helpline": TENANT_SUPPORT_PHONE
                        }
                ...
                with col_st2:
                    if st.button("🟢 RE-ACTIVATE LINE / MARK ACTIVE", use_container_width=True):
                        ...
                        if tenant_meta.get("wa_enabled") and tenant_meta.get("wa_instance_id") and tenant_meta.get("wa_token"):
                            ...
                            try:
                                requests.post(url, json={"chatId": f"{clean_phone}@c.us", "message": msg}, headers={'Content-Type': 'application/json'}, timeout=5)
                            except:
                                pass
                ...
                with get_db_connection() as conn:
                    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                        cur.execute("""
                            SELECT invoiceid, amountpaid, remainingarrears, discountgiven, currentpackage, datetimestamp, paymentmethod FROM billing_history
                            WHERE customerid = %s AND tenant_id = %s AND transactiontype = 'BILL_PAYMENT'
                    ...
                    if st.button("🚨 REVERSE & UNDO THIS TRANSACTION", type="primary", use_container_width=True):
                        with get_db_connection() as conn:
                            with conn.cursor() as cursor:
                                ...
                                try:
                                    base_bill = int(float(str(node_sr.get('billamount', 0))))
                                exception:
                                    base_bill = 1500
            ...
            with get_db_connection() as conn:
                with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                    query = """
                        ...
                        WHERE tenant_id = %s AND transactiontype = 'BILL_PAYMENT' AND LEFT(datetimestamp, 10) >= %s AND LEFT(datetimestamp, 10) <= %s
...
# ==========================================
    ...
    if str(st.session_state.get('user_role', '')).lower() not in ["owner", "admin"]:
        ...
        with adm_tabs[5]:
            ...
            try:
                with get_db_connection() as conn:
                    with conn.cursor(cursor_factory=ps psycopg2.extras.RealDictCursor) as cur:
                        if is_master_owner:
    ...
    if portal_tenant and portal_input:
        ...
        if not c_rows:
            ...
            try:
                ...
                bill_amt_val = 0; balance_shift_val = 0
